...
\begin{dem}
    ...
    Viendo la sucesión de Mayer-Vietoris para $A$ y $B$, el término $H_i(A)\oplus H_i(B)$ se anula para todo $i\in\N$, entonces obtenemos las siguientes sucesiones exactas:

    $$0\rightarrow H_i(A\cap B)\xrightarrow{\varphi_i} H_{i-1}(A\cup B)\rightarrow 0$$

    De igual forma que en la prueba anterior, los morfismos $\varphi_i$ van a ser isomorfismos para todo $i\in\N$.

    Los conjuntos $A$ y $B$ cumplen que $A\cap B=\s^n - h(\s^k)$ y $A\cup B=\s^n-h(\s^{k-1})$. Por lo tanto, obtuvimos que $H_i(\s^n - h(\s^k))\simeq H_{i-1}(\s^n-h(\s^{k-1}))$ para todo $i\in\N$.

    Por la hipótesis de inducción, sabemos que $H_{i-1}(\s^n-h(\s^{k-1}))$ es $\Z$ si $i-1 = n- (k-1) -1$ y $0$ en cualquier otro caso. O sea, es $\Z$ si $i=n-k+1$
    
    Por lo tanto, trasladando esto a $H_i(\s^n - h(\s^k))$ tenemos que es $\Z$ si.
    
    Despejando, tenemos que $H_i(\s^n - h(\s^k))$ es $\Z$ si $i=n-k+1$ y $0$ en cualquier otro caso.
