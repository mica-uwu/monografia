...
Definimos la función $h\colon\Omega(X,x_0)\to C_1(X)$ que manda un lazo $f\in\Omega(X,x_0)$ en un $1$-símplice singular, precomponiendo con la proyección $\Delta^1\mapsto\s^1$. Observar que $\partial h(f)=0$. En lo que sigue vamos a probar que si $f\simeq g$ entonces $h(f)\sim h(g)$, y que $\tilde{h}$ manda el producto en $\pi_1(X,x_0)$ a la suma en $H_1(X)$. Por lo tanto $h$ baja al cociente como un morfismo $\tilde{h}:\pi_1(X,x_0)\to H_1(X,x_0)$.
...
Como $H_1(X)$ es abeliano, entonces el conmutador de $pi_1(X)$ se anula por el morfismo. Veamos que el núcleo de $\tilde{h}$ es exactamente el conmutador de $pi_1(X)$. 

Sea $[f]\in\pi_1(X)$ tal que $\tilde{h}([f])=0$, entonces $f$ vista como una $1$-cadena es homóloga a $0$, es decir, existe una $2$-cadena $\sum n_i\sigma_i$ tal que $\partial\left(\sum n_i\sigma_i\right)=f$.
