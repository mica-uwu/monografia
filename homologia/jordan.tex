...
\comm{obviamente $k\leq n$, esto usa invariancia del dominio, q todo encaje va a cumplir $k\leq n$. agregar esto después}
...
\end{proof}
    ...
    Por el primer lema que probamos en esta sección, sabemos que $\tilde{H}_0(\s^n - h(D'))=0$, es decir, $H_0(\s^n - h(D'))=\Z$, y por lo tanto $\s^n - h(D')$ tiene una única componente conexa por caminos. Como $\s^n$ es localmente conexa por caminos, y $\s^n - h(D')$ es abierto, entonces $\s^n - h(D')$ es conexo.

    Sin embargo, podemos descomponer $\s^n - h(D') = A\sqcup B\sqcup h(D)$, y estos son tres abiertos disjuntos, lo cual nos da el absurdo.
