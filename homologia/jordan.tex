...
\end{dem}
    ...
    \begin{obs}
    En el caso de codimensión 1, este teorema nos dice que $\tilde{H_i}(\s^n-h(\s^{n-1}))$ es $\Z$ si $i=0$ y $0$ en cualquier otro caso. Esto significa que $H_0(\s^n-h(\s^{n-1}))=\Z\oplus\Z$, es decir, que $\s^n-h(\s^{n-1})$ tiene dos componentes conexas por caminos. Además, como el resto de los grupos de homología son triviales, estas componentes son bolas homológicas. 
